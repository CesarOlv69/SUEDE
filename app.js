...
async function loadCardEdits() {
  var data = await sbFetch(SB_CARDS + '?select=card_id,title,note,time,photo');
  if (data && Array.isArray(data)) {
    cardEditsCache = {};
...
function closeCardEdit() {
  editingCardId = null;
  cardEditPhoto = null;
  var m = document.getElementById('card-edit-modal');
...
async function saveCardEdit() {
  ...
  if (timeVal) {
    ...
    if (tlItem2 && tlItem2.parentElement) sortTimeline(tlItem2.parentElement);
  }
